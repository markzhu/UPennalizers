...
h.get_labelB = @get_labelB;
    ...
    function r = get_monitor_struct()
        ...
        labelBm = labelAm/4;
        freeHorizon = h.vcmImage.get_horizonA();
        labelAn = h.vcmImage.get_height()/4;
        freeHorizon = freeHorizon - labelAn;
		freehorizonDir = h.vcmFreespace.get_horizonA();
        r.free = struct('Ax',freeValueA(1:labelAm),...
                        'Ay',freeValueA(labelAm+1:end),...
                        'Bx',freeValueB(1:labelBm),...
                        'By',freeValueB(labelBm+1:end),...
