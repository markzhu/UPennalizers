...
--]]
  ...
  pTorso[3]= calculate_body_height(ph);
...
function calculate_body_height(ph)

   footHeight=0.0355;
   legLength = 0.093+0.093;
   hipOffsetX = 0.008;
   hipOffsetY = 0.037;
   hipOffsetZ = 0.096;

   local uHipL = util.pose_global(
	vector.new({hipOffsetX*math.cos(bodyTilt)+		
		    hipOffsetZ*math.sin(bodyTilt),
		    hipOffsetY,0}),
	uTorsoActual);
   local uHipR = util.pose_global(
	vector.new({hipOffsetX*math.cos(bodyTilt)+		
		    hipOffsetZ*math.sin(bodyTilt),
		    -hipOffsetY,0}),
	uTorsoActual);
   
   local distL = (uLeft[1]-uHipL[1])^2 + (uLeft[2]-uHipL[2])^2;
   local distR = (uRight[1]-uHipR[1])^2 + (uRight[2]-uHipR[2])^2;

   extendLeg = 0;
   if distL<distR then --Left leg is extended leg
      extendedLeg = 0;
   else
      extendedLeg = 1;
   end

--   extendLeg = supportLeg;

   legHeight = 0;
   if extendedLeg==0 then
     legHeight = math.sqrt(legLength^2 - distL);


print("LS uLeft  / uRight",uLeft[1]-uTorsoActual[1],distL,
	uRight[1]-uTorsoActual[1],distR);


   else
     legHeight = math.sqrt(legLength^2 - distR);

print("RS uLeft  / uRight",uLeft[1]-uTorsoActual[1],distL,
	uRight[1]-uTorsoActual[1],distR);
   ...
   local base = footHeight + 
	math.sin(bodyTilt)*hipOffsetX + math.cos(bodyTilt)*hipOffsetZ;
print("bodyHeight:",base+legHeight)  
--   return 0.295;
   return base + legHeight;
end
