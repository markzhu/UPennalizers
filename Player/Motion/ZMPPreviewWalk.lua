...
uTorsoF = vector.new({supportX, 0, 0});
uLeftF = vector.new({0, footY, 0});
uRightF = vector.new({0, -footY, 0});
supportLegF = 0; 
...
--[[
  fsr=Body.get_sensor_fsr();
  print("FSR L ",unpack(fsr[1]));
  print("FSR R ",unpack(fsr[2]));

  zmp=Body.get_sensor_zmp();
  if zmp[1]~=2 then
    print("Measured ZMP:",unpack(zmp[2]))
  end
--]]
...
function update_zmp_array()
  t = Body.get_time();
  local iStep, ph = math.modf((t-t0)/tStep);

  if (iStep>iStep0) then
    if stopRequest>0 then --stop
      supportLegF = 2; --Double support end
      uSupportF = util.se2_interpolate(0.5,uLeftF1,uRightF1);
      uLeftF1 = uLeftF2;
      uRightF1 = uRightF2;
...
function update()
  ...
  iStep2, ph2 = math.modf((t-t1)/tStep);
  xFoot, zFoot = foot_phase(ph2);  
  ...
  x_err = {math.sin(imuPitch)*bodyHeight,
    	   -math.sin(imuRoll)*bodyHeight};

  feedback_gain = 0.5;
  x_closed=x[1][1]+x_err[1]*feedback_gain;
  y_closed=x[1][2]+x_err[2]*feedback_gain;
